...
parser.add_argument('--indim', type=int, default=166, help='feature dim')
parser.add_argument('--nroi', type=int, default=166, help='num of ROIs')
...
save_path = os.path.join(opt.dataset_dir, '{}_roi'.format(opt.dataset_name), 'braingnn_{}'.format(opt.atlas))
...
fold = opt.fold
writer = SummaryWriter(os.path.join(opt.save_path, './log', str(fold)))

pkl_name = os.path.join(save_path, 'valid_subject_list.pkl')
with open(pkl_name, 'rb') as file:
    subject_IDs = pickle.load(file)
object_num = len(subject_IDs)

################## Define Dataloader ##################################
dataset = ABIDEDataset(save_path, name)
dataset.data.y = dataset.data.y.squeeze()
dataset.data.x[dataset.data.x == float('inf')] = 0

tr_index, val_index, te_index = train_val_test_split(object_num=object_num, fold=fold)
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        loss_c = F.nll_loss(output, data.y, ignore_index=-1)
        ...
        loss = opt.lamb0*loss_c + opt.lamb1 * loss_p1 + opt.lamb2 * loss_p2 + opt.lamb3 * loss_tpk1 + opt.lamb4 *loss_tpk2 + opt.lamb5* loss_consist
...
def test_loss(loader,epoch):
    ...
    for data in loader:
        ...
        + opt.lamb1 * loss_p1 + opt.lamb2 * loss_p2 + opt.lamb3 * loss_tpk1 + opt.lamb4 *loss_tpk2 + opt.lamb5* loss_consist
